...
def financial_bound():
    ...
    return project(include=["financial"], T=1, H=32, W=32, d_model=32)


@pytest.fixture
def macro_bound():
    """A macro-only projection."""
    return project(include=["country_us.macro"], T=1, H=24, W=24, d_model=32)


@pytest.fixture
def full_bound():
    """Full world model (expensive, use sparingly)."""
    return project(include=["*"], T=1, H=160, W=160, d_model=64)
